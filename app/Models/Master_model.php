...
class Master_model extends Model
{
	...
	*/
	...
	public function getRecords($tbl_name,$condition=array(),$select=FALSE,$order_by=array(),$start=FALSE,$limit=FALSE,$like=array(),$group_by = array())
	{
	...
	*/
	...
	public function fSendMail($to, $to_name, $message_txt, $subject, $from_email = null, $attachment_path = null)
	{
    require_once(FCPATH . '/vendor/autoload.php'); // Ensure autoload path is correct

    // Load API Key from CI4 environment file
    $apiKey = SENDINBLUE_API;

    	if (!$apiKey) {
        log_message('error', 'SendinBlue API Key is missing in .env file');
        return false;
    	}
        ...
        $sendSmtpEmail = new \SendinBlue\Client\Model\SendSmtpEmail([
            'subject' => $subject,
            'htmlContent' => $message_txt,
            'sender' => [
                'name' => $from_name ?? 'SpyClass',
                'email' => $from_email ?? getenv('ADMIN_EMAIL')
            ],
            'to' => [
        ...
        if ($attachment_path && file_exists($attachment_path)) {
            $sendSmtpEmail['attachment'] = [
                [
                    'name' => basename($attachment_path), // Get file name
                    'content' => base64_encode(file_get_contents($attachment_path)) // Encode file content
                ]
            ];
        }
	try {
        ...
        log_message('error', 'SendinBlue Email Error: ' . $e->getMessage());
        return false;
    }
	}
	...
	public function fSendMailOLD($sToEmail = null, $sMessage = null, $sSubject = null, $attachmentPath = null)
    {
        ...
        $email->initialize([
            'protocol' => !empty($aEmailConfiguration) ? $aEmailConfiguration['email_protocol'] : 'smtp',
            'SMTPHost' => !empty($aEmailConfiguration) ? $aEmailConfiguration['smtp_host'] : 'smtp.hostinger.com',
            'SMTPPort' => !empty($aEmailConfiguration) ? intval($aEmailConfiguration['smtp_port']) : '587',
            'SMTPUser' => !empty($aEmailConfiguration) ? $aEmailConfiguration['smtp_user'] : 'info@zeplinix.com',
            'SMTPPass' => !empty($aEmailConfiguration) ? $aEmailConfiguration['smtp_password'] : 'admin@3Tytdfsd887',
            'mailType' => !empty($aEmailConfiguration) ? $aEmailConfiguration['smtp_crypto_type'] : 'tls',
            'mailType' => 'html',
        ]);
    ...
    function convertDateFormat($dateInMMDDYYYY) {
	    ...
	    $date = $dateInMMDDYYYY->format('m-d-Y');
