        ...
        <div class="card invoice-form">
            <div class="card-header  invoice-header-bg ui-sortable-handle" style="border-radius:16px 16px 0px 0px">
                ...
                <div class="card-tools ">
                    <a href="<?= site_url($panel.'/invoice/file/'.$GetInvoiceData['invoice_file']); ?>" target="blank">
        ...
        async function sendInvoiceEmail(invoiceId, invoiceFile, defaultEmail) {
            ...
            if (email) {
                Swal.fire({
                    ...
                    text: `Are you sure you want to send the invoice to ${email}?`,
                    icon: "question",
                    showCancelButton: true,
                    ...
                    if (result.isConfirmed) {
                        ...
                        fetch('<?= site_url($panel . "/invoice/sendEmail") ?>', {
                            method: 'POST',
                            headers: {
                        ...
                        .then(data => {
                            if (data.status === 'success') {
                                ...
                                Swal.fire({
                                    icon: "error",
                                    title: "Error",
                                    text: `Failed to send invoice: ${data.message}`,
                                });
                            }
                        })
                        .catch(error => {
