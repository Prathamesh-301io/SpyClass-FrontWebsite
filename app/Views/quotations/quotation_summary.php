    ...
    <div class="card-body">
        <div class="card invoice-form">
            <div class="card-header  invoice-header-bg ui-sortable-handle" style="border-radius:16px 16px 0px 0px">
                <h3 class="card-title text-left"><strong>Share Quotation</strong></h3>
                <div class="card-tools ">
                    <a href="<?= site_url($panel.'/quotations/file/'.$GetInvoiceData['invoice_file']); ?>" target="blank">
                   ...
                   <!--  <button type="button" class="btn btn-tool bg-white" onclick="printDiv('InvoicePDF');"><i class="fas fa-print"></i> Print</button> -->
                    <button type="button" class="btn btn-tool bg-white"  onclick="sendQuotationEmail('<?= $GetInvoiceData['id']; ?>', '<?= $GetInvoiceData['invoice_file']; ?>', '<?= $ClientData['company_email']; ?>')"><i class="fas fa-envelope"></i> Email </button>
...
<?= $this->section('extra_script') ?>
    <script>
        ...
        async function sendQuotationEmail(invoiceId, invoiceFile, defaultEmail) {
            ...
            if (email) {
                Swal.fire({
                    ...
                    text: `Are you sure you want to send the quotation to ${email}?`,
                    icon: "question",
                    showCancelButton: true,
                    ...
                    if (result.isConfirmed) {
                        Swal.fire({
                            title: "Sending...",
                            text: "Please wait while the quotation is being sent.",
                            allowOutsideClick: false,
                            didOpen: () => {
                                Swal.showLoading();
                            }
                        });
                        fetch('<?= site_url($panel . "/quotations/sendEmail") ?>', {
                            method: 'POST',
                            headers: {
                        ...
                        .then(data => {
                            if (data.status === 'success') {
                                ...
                                Swal.fire({
                                    icon: "error",
                                    title: "Error",
                                    text: `Failed to send quotation: ${data.message}`,
                                });
                            }
                        })
                        .catch(error => {
                            ...
                            Swal.fire({
                                icon: "warning",
                                title: "Error",
                                text: "An error occurred while sending the quotation.",
                            });
                        });
                    }
                });
